...
export async function exchangeAccessCodeForCredentials(
  ...
  return axios.post<DiscordOAuth2CredentialsResponse>(
    TWITTER_API_ROUTES.OAUTH2_TOKEN,
    payload,
    axiosConfig
  );
}

export async function getTwitterUserDetails(accessToken: string) {
  return axios.get<DiscordOAuth2UserResponse>(
    TWITTER_API_ROUTES.OAUTH2_USER,
    authHeaders(accessToken)
  );
}

export async function createUser(params: CreateUserParams) {
  const userDB = await userRepository.findOne({ twitterId: params.discordId });
...
export async function updateUser(user: User, params: CreateUserParams) {
  ...
  user.tag = params.tag;
  user.twitterUsername = params.username;
  user.avatar = params.avatar;
  user.twtAccessToken = params.accessToken;
  user.twtRefreshToken = params.refreshToken;
...
export async function revokeToken(accessToken: string) {
  ...
  const formData = new url.URLSearchParams({
    ...
    client_secret: process.env.TWITTER_OAUTH_SECRET!,
    token: decryptedToken,
  });
  return axios.post(
    TWITTER_API_ROUTES.OAUTH2_TOKEN_REVOKE,
    formData.toString(),
    axiosConfig
  );
}
